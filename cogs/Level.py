...
cursor.execute(
    """CREATE TABLE IF NOT EXISTS Level(user_id INT PRIMARY KEY, exp INT, level INT, last_level INT)"""
)
...
class Leveling(commands.Cog):
    ...
    async def server_leaderboard(self, ctx):
        ...
        embed = discord.Embed(
            title="🏆 Top 25 Users of the Server", color=discord.Color.gold()
        )
        ...
        for row in rows:
            ...
            embed.add_field(
                name=f"{rank}. {user.name}", value=f"**Score**: {exp}", inline=False
            )
...
class Leveling_Debugger(commands.Cog):
    ...
    @commands.hybrid_group(name="exp", hidden=True)
    @commands.is_owner()
    ...
    async def remove_exp(
        self, ctx, exp_amount: Optional[str], set_level: Optional[int]
    ):
        if ctx.author.bot:
            return
        exp, level, last_level = fetch_user_info(ctx)
        ...
        if level < last_level:
            await self.BOT_CMD_CHANNEL.send(
                f"Fiery, you leveled down, ab exp hain {exp}"
            )
