...
class Responses(commands.Cog):
    ...
    async def _8ball_command(self, ctx):
        ...
        ball_responses = [
            "The same response as every boy/girl gave you when you asked them out, no.",
    ...
    async def ping(self, ctx):
        ...
        embed.set_author(name=str(ctx.author).title(), url="https://github.com/GuBiiz")
        await ctx.send(embed=embed)
    ...
    async def slap_command(self, ctx, member: discord.Member, reason: str):
        ...
        await ctx.send(
            f"{ctx.author.mention} slapped {member.mention} with reason {reason}"
        )

        if randint_1 == randint_2:
            await ctx.send(
                f"***CRITICAL HIT*** {member.mention} is now muted for 5 minutes as they are unconscious from the slap"
            )
            await member.add_roles(discord.utils.get(member.guild.roles, name="Muted"))
            await sleep(300)
            await ctx.send(
                f"{member.mention} welcome back! You were slapped unconscious from the slap given by {ctx.author.mention}"
            )
