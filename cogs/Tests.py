...
cogs = Literal[
    ...
    "cogs.Tests",
    "cogs.Responses",
    "hypixel.catacombs.Catacombs",
    "cogs.Level",
    "hypixel.Emoji",
    "hypixel.fishing.Fishing",
    "hypixel.general.Skills",
    "hypixel.general.Accountlinks",
    "hypixel.partyfinder.Party",
]
...
class Tests(commands.Cog):
    ...
    async def testers(self, ctx):
        if ctx.invoked_subcommand is None:
            await ctx.reply("No subcommand used", ephemeral=True)
    ...
    async def testers_error(self, ctx, error):
        if isinstance(error, commands.errors.MissingPermissions):
            await ctx.send(
                "You do not have the required permissions to use this command, spread this command and you will be banned "
